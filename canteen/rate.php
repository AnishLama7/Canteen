<?php 
	...
	if(isset($POST['starRate'])) {
		...
		$sql = $conn->prepare("SELECT * FROM review WHERE userName=?");
		...
		$stmt = $conn->prepare("INSERT INTO review (userName, userReview, userMessage, dateReviewed) values (?, ?, ?, ?)");
		...
		$stmt = $conn->prepare("UPDATE review SET userName=?, userReview=?, userMessage=?, dateReviewed=? WHERE userName=?");
